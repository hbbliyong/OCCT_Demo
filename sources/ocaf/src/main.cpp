...
public:
	...
	void SetColor(const unsigned r, const unsigned g, const unsigned b) {
		...
		TDataStd_Integer::Set(m_root, color);
	}
...
int main() {
	...
	TDF_Label partLabel = doc->Main();
	TDF_Label meshsLabel = partLabel.Father().FindChild(2);

	IPart part1(TDF_TagSource::NewChild(partLabel));
	IPart part2(TDF_TagSource::NewChild(partLabel));
	IPart part3(TDF_TagSource::NewChild(partLabel));
	...
	//Create meshes
	IMesh mesh(TDF_TagSource::NewChild(meshsLabel));
	mesh.Create(part2);

	PCDM_StoreStatus sstatus = app->SaveAs(doc, "D:/test1.cbf");
