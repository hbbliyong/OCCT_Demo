...
TopoDS_Shape BuildWheelAxle(const TopoDS_Shape& wheel, const TopoDS_Shape& axle, const double L) {
	TopoDS_Compound comp;
	BRep_Builder builder;
	builder.MakeCompound(comp);

	gp_Trsf wheelRight;
	wheelRight.SetTranslationPart(gp_Vec(L / 2, 0, 0));

	gp_Trsf wheelLeft;
	gp_Quaternion qn(gp::DY(), M_PI);
	gp_Trsf R;
	R.SetRotation(qn);
	wheelLeft = wheelRight.Inverted() * R;

	builder.Add(comp, wheel.Moved(wheelRight));
	builder.Add(comp, wheel.Moved(wheelLeft));
	builder.Add(comp, axle);
...
TopoDS_Shape BuildChassis(const TopoDS_Shape& wheelAxle, const double CL) {
	...
	gp_Trsf frondTrans, backTrans;
	frondTrans.SetTranslationPart(gp_Vec(0, CL / 2, 0));
	backTrans.SetTranslationPart(gp_Vec(0, -CL / 2, 0));

	builder.Add(comp, wheelAxle.Moved(frondTrans));
	builder.Add(comp, wheelAxle.Moved(backTrans));
...
struct TWheelProtoType:public TProtoType
{
	TopoDS_Face frontFace;
	TDF_Label frontFaceLab;
};


bool WriteStep(const Handle(TDocStd_Document)& doc, char* fileName) {
...
int main(int argc, char** argv)
{
	...
	TWheelProtoType wheelProto;
	wheelProto.shape = BuildWheel(OD, W);
	wheelProto.label = ST->AddShape(wheelProto.shape, false);

	TProtoType axleProto;
	axleProto.shape = BuildAxle(D, L);
	axleProto.label = ST->AddShape(axleProto.shape, false);

	TProtoType wheel_axle_proto;
	wheel_axle_proto.shape = BuildWheelAxle(wheelProto.shape, axleProto.shape, L);
	wheel_axle_proto.label = ST->AddShape(wheel_axle_proto.shape, true);

	TProtoType ChassisProto;
	ChassisProto.shape = BuildChassis(wheel_axle_proto.shape, CL);
	...
	TDataStd_Name::Set(wheel_axle_proto.label, "WheelAxle");
	TDataStd_Name::Set(ChassisProto.label, "Chassis");

	for (TDF_ChildIterator itor(ChassisProto.label); itor.More(); itor.Next()) {
		TDataStd_Name::Set(itor.Value(), "wheel-axle-instance");
	}

	CT->SetColor(wheelProto.label, Quantity_Color(1, 0, 0, Quantity_TOC_RGB), XCAFDoc_ColorGen);
	CT->SetColor(axleProto.label, Quantity_Color(0, 1, 0, Quantity_TOC_RGB), XCAFDoc_ColorGen);
	...
	wheelProto.frontFaceLab = ST->AddSubShape(wheelProto.label, wheelProto.frontFace);

	CT->SetColor(wheelProto.frontFaceLab, Quantity_Color(0, 0, 1, Quantity_TOC_RGB), XCAFDoc_ColorSurf);

	PCDM_StoreStatus status = app->SaveAs(doc, "D:/test2.xbf");
